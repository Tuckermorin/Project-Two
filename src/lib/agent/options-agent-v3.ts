...
async function ragCorrelationScoring(state: AgentState): Promise<Partial<AgentState>> {
  ...
  for (const candidate of state.candidates) {
    try {
      ...
      // - Yield: 30% (ROI potential)
      // - IPS: 40% (factor compliance)
      // - RAG: 30% (historical win rate)
      const compositeScore = historicalAnalysis.has_data
        ? (yieldScore * 0.3) + (ipsScore * 0.4) + (historicalAnalysis.win_rate * 100 * 0.3)
        : (yieldScore * 0.4) + (ipsScore * 0.6); // No RAG: increase yield + IPS weight
...
// ============================================================================
// STEP 11.5: AI-Enhanced Trade Evaluation with Live Intelligence
// ============================================================================
// NOTE: This step is now imported from ./ai-enhanced-evaluation-step.ts
// The evaluateTradesWithAI function uses the Phase 3 Enhanced Recommendation Service
...
export function buildAgentV3Graph() {
  ...
  graph.addNode("AIEnhancedEvaluation", evaluateTradesWithAI);
  ...
  graph.addEdge("SortTop5", "AIEnhancedEvaluation");
  graph.addEdge("AIEnhancedEvaluation", "Diversification");
