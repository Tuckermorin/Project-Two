...
export class IPSBacktestingEngine {
  ...
  constructor(config: BacktestConfig, progressCallback?: (progress: BacktestProgress) => void) {
    this.config = config;
  ...
  /**
   * Calculate trade outcome by simulating expiration
  ...
  private async calculateOutcome(option: any): Promise<Partial<TradeMatch>> {
    // Simplified: Assume credit spread with 50% profit target
    // In reality, you'd track the option price over time to expiration

    const entryPremium = option.mark || ((option.bid + option.ask) / 2);
    const profitTarget = entryPremium * 0.5; // Take profit at 50%

    // Simulate: If delta moved favorably, we win. Otherwise, loss.
    // This is a placeholder - ideally you'd track daily price movements
    const randomOutcome = Math.random();
    const winProbability = 0.65; // Base 65% win rate for credit spreads

    const won = randomOutcome < winProbability;

    if (won) {
      const pnl = profitTarget * 100; // 100 shares per contract
      const roi = 50; // 50% profit target

      return {
        exitDate: new Date(option.snapshot_date),
        exitPrice: entryPremium - profitTarget,
        realizedPnl: pnl,
        realizedRoi: roi,
      ...
      const pnl = -entryPremium * 100; // Max loss
      const roi = -100;
  ...
  private async calculateResults(trades: TradeMatch[]): Promise<BacktestResults> {
    const closedTrades = trades.filter(t => t.actualOutcome && t.actualOutcome !== "pending");
